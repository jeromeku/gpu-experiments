/*
    Benchmarks:
        - 4096x4096x4096 : 1135.72 TFLOp/s
        - 8192x8192x8192 : 2552.22 TFLOp/s
        - 16384x16384x16384 : 2655.03 TFLOp/s
        - 204800x2048x1536 : 2259.39 TFLOp/s
*/
...
struct mxfp8_matmul_base {
    ...
    struct globals {
        using A_gl = gl<fp8e4m3, 1, 1, -1, -1, A_tile>; // M x K
        using B_gl = gl<fp8e4m3, 1, 1, -1, -1, B_tile>; // N x K
        using C_gl = gl<bf16,    1, 1, -1, -1, C_tile>; // M x N
        using A_sc_gl = gl<fp8e8m0, 1, -1, -1, SCALE_BLOCK, A_sc_vec>; // (M / 128) x (K / 128) x 512
        using B_sc_gl = gl<fp8e8m0, 1, -1, -1, SCALE_BLOCK, B_sc_vec>; // (N / 128) x (K / 128) x 512
    ...
    template <bool do_load, bool load_B1>
    ...
    template <bool do_store, bool store_C1>
    ...
    __device__ static inline void main_loop(const globals &G, state &S) {
        ...
        for (int block_idx = clusterIdx().x; block_idx < num_blocks; block_idx += gridDim.x / config::CLUSTER_SIZE) {
            ...
            if (S.warpgroup_id == config::NUM_CONSUMERS && S.lane_id == 0) {
                if (S.warp_id == TILE_LOADER) {
                    if (load_A && load_B)
                        tile_loader_loop<true, true>(G, S);
                    else if (load_A)
                        tile_loader_loop<true, false>(G, S);
                    else if (load_B)
                        tile_loader_loop<false, true>(G, S);
                    else
                        tile_loader_loop<false, false>(G, S);
                } else if (S.warp_id == SCALE_SM_LOADER) {
                    if (do_load_store && load_store_BC1)
                        scale_sm_loader_loop<true, true>(G, S);
                    else if (do_load_store)
                        scale_sm_loader_loop<true, false>(G, S);
                    else
                        scale_sm_loader_loop<false, false>(G, S);
                } else if (S.warp_id == SCALE_TM_LOADER) {
                    scale_tm_loader_loop(G, S);
                } else if (S.cta_id == 0 && S.warp_id == MMA_LAUNCHER) {
                    mma_launcher_loop(G, S);
                }
            } else if (S.warpgroup_id < config::NUM_CONSUMERS) {
                if (do_load_store && load_store_BC1)
