...
A_t_fp8 = torch.zeros(N, M, dtype=torch.float8_e4m3fn, device="cuda")
A_t_sc = torch.zeros(N // 128, M // 128, 512, dtype=torch.uint8, device="cuda")
...
# Generate PyTorch transposed reference
A_t_fp8_ref, _A_t_sc_ref = quantize_2d(A.T)
A_t_sc_ref = reshape_sc(_A_t_sc_ref)
...
kernel(A, A_fp8, A_sc, A_t_fp8, A_t_sc)
...
abs_diff = torch.abs(A_t_fp8_ref.to(torch.float32) - A_t_fp8.to(torch.float32))
print('Max adiff (Kernel_T-FP8):', abs_diff.max().item())
print('Mean adiff (Kernel_T-FP8):', abs_diff.mean().item())
abs_diff = torch.abs(A_t_sc_ref.to(torch.float32) - A_t_sc.to(torch.float32))
