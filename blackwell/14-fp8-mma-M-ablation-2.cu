/*
    Another version of the previous test, where we run 3 separate
    matmul pipelines in parallel, essentially doing 384x128 matmuls

    Hypothesis is that this is better since:
        - It utilizes more tensor cores (1.5x compared to 256x128)
        - It is still easy to handle non-multiple M inputs

    Observation:
        - Benefit is minimal. About 40 more TFLOPs
        - Pure M=256 K=128 N=128 : 2640 TFLOPs (previous test)
        - Pure M=384 K=128 N=128 : 2684 TFLOPs (this test)

        => At the current stage, I don't think it's worth the hassle.
*/
...
struct globals {
    ...
    static constexpr int ROW_BLOCK = 384;
    static constexpr int COL_BLOCK = 128;
    static constexpr int REDUCTION_BLOCK = 128;

    using A_tile = st_fp8e4m3<ROW_BLOCK / 3, REDUCTION_BLOCK>;
    using B_tile = st_fp8e4m3<COL_BLOCK, REDUCTION_BLOCK>;
    using C_tile = st_bf<ROW_BLOCK / 12, COL_BLOCK>;
...
void kernel(const __grid_constant__ globals G) {
    ...
    if (threadIdx.x == 0) {
        for (int i = 0; i < config::PIPELINE_STAGES; ++i) {
            ...
            init_semaphore(inputs_finished[i], 0, 3);
        }
    ...
    if (warpgroup_id == config::NUM_WARPGROUPS - 1) {
        ...
        if (warp_id == 3 && lane_id == 0) {
            ...
            for (int block_idx = blockIdx.x; block_idx < num_blocks; block_idx += gridDim.x) {
                ...
                for (int i = 0; i < num_iters_per_block; ++i) {
                    ...
                    tma::load_async(inputs[stage].A[0], G.A, {row_block_idx * 3 + 0, i}, inputs_arrived[stage]);
                    tma::load_async(inputs[stage].A[1], G.A, {row_block_idx * 3 + 1, i}, inputs_arrived[stage]);
                    tma::load_async(inputs[stage].A[2], G.A, {row_block_idx * 3 + 2, i}, inputs_arrived[stage]);
            ...
            arrive(outputs_arrived);
        } else if (warp_id == 0 && lane_id < 3) {
            // Producer group -- launchers
            using tm_t = tt<float, globals::ROW_BLOCK / 3, globals::COL_BLOCK>;
        ...
        for (int block_idx = blockIdx.x; block_idx < num_blocks; block_idx += gridDim.x) {
            ...
            rt_fl<globals::ROW_BLOCK / 12, globals::COL_BLOCK> C_reg;
            warpgroup::load_async(C_reg, tm);
            tensor_load_wait();
            consumer::sync(1); // wait for all consumer WGs, to arrive asap
            ...
            for (int i = 0; i < 12; ++i) {
                if (consumer::warpid() == i) {
                    ...
                    __syncwarp();
                    if (lane_id == 0) {
                        tma::store_async(G.C, outputs.C, {row_block_idx * 12 + i, col_block_idx});
