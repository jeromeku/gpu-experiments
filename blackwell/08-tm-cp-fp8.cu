/*
    Observations:
    - Tensor memory is little-endian
*/
...
struct globals {
    ...
    gl<fp8e8m0, 1, 1, 1, BLOCK_SIZE * BLOCK_SIZE / 32> in;
    gl<int, 1, 1, 128, 128> tm; // preserve the TM layout
...
__global__ void kernel(const __grid_constant__ globals G) {
    ...
    fp8e8m0 *st = reinterpret_cast<fp8e8m0 *>(
        (reinterpret_cast<uint64_t>(&smem[0]) + 1023) / 1024 * 1024
    );
    if (threadIdx.x == 0) {
        for (int i = 0; i < globals::BLOCK_SIZE * globals::BLOCK_SIZE / 32; ++i) {
            st[i] = G.in.raw_ptr[i];
        }
    }
    __syncthreads();
    ...
    for (int i = 0; i < globals::BLOCK_SIZE; i++)
        G.tm.raw_ptr[threadIdx.x * globals::BLOCK_SIZE + i] = dst[i];
    if (threadIdx.x < 32)
