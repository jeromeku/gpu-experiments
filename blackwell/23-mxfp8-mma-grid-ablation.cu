/*
    Q: Is persistent grid truly beneficial (for matmul in this case. Obviously would be different for other tasks)?

    Observations:
        Persistent grid: 2357.06 TFLOp/s
        Variable grid w/ supergrouping: 2334.89 TFLOp/s
        Naive variable grid: 2245.67 TFLOp/s

    A: It is beneficial to use persistent grids, but even more beneficial is to consider the L2 cache util
*/
...
struct globals {
    ...
    __host__ inline dim3 grid() {
        int has_tail = C.rows() % ROW_BLOCK == ROW_BLOCK / 2;
        int num_blocks_per_row = C.cols() / COL_BLOCK;
        int num_blocks_per_col = C.rows() / ROW_BLOCK + has_tail;
        return dim3(num_blocks_per_row, num_blocks_per_col); 
    }
...
do { \
    [[maybe_unused]] int row_block_idx = blockIdx.y; \
    [[maybe_unused]] int col_block_idx = blockIdx.x; \
    {__VA_ARGS__} \
} while (0)
...
void kernel(const __grid_constant__ globals G) {
    ...
    uint32_t A_sc_tm_addr[config::PIPELINE_STAGES][2] = {
        {tm_addr + 256 +  0, tm_addr + 256 + 16},
        {tm_addr + 256 + 32, tm_addr + 256 + 48},
        {tm_addr + 256 + 64, tm_addr + 256 + 80},
        {tm_addr + 256 + 96, tm_addr + 256 + 112}
    };
    ...
    if (threadIdx.x == 0) {
        for (int i = 0; i < config::PIPELINE_STAGES; ++i) {
            ...
            init_semaphore(inputs_finished[i], 0, 2); // 2 matmul pipelines
            init_semaphore(scales_arrived[i], 0, 1);
            init_semaphore(scales_finished[i], 0, 2); // 2 matmul pipelines
        }
    ...
    if (warpgroup_id == config::NUM_WARPGROUPS - 1) {
        ...
        if (warp_id == 3 && lane_id == 0) {
            ...
            WORKER_OUTER_LOOP({
                WORKER_INNER_LOOP({
                    ...
                    wait(inputs_finished[stage], get_phasebit<1>(phasebits, stage));
                    update_phasebit<1>(phasebits, stage);
                    load_scales_to_tensor_memory(A_sc_tm_addr[stage][0], &input_scales[stage].A[0][0]);
                    load_scales_to_tensor_memory(A_sc_tm_addr[stage][1], &input_scales[stage].A[1][0]);
            ...
            WORKER_OUTER_LOOP({
                WORKER_INNER_LOOP({
                    ...
                    launch_128x128_mxfp8_matmul(out_tm_addr[lane_id], 
                                                input_tiles[stage].A[lane_id], input_tiles[stage].B, 
                                                A_sc_tm_addr[stage][lane_id], B_sc_tm_addr[stage],
                                                i == 0 ? 0 : 1);
        ...
        WORKER_OUTER_LOOP({
            ...
            tt<float, 128, 128> tm(out_tm_addr[warpgroup_id]);
