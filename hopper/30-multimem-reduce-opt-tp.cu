/*
    ...
    Final version of multimem all reduce
    Matches the theoretical maximum NVLink Gen4 bandwidth of 450 GB/s (per direction)

    This is how we should use multimem in TK.

    Numbers:
    - 8 dev, 1GiB allreduce: 4.438495 ms (0.4ms of this is threading and sync stuff like locks/conds)
    - Note that the same 8 dev 1GiB allreduce with a single device issuing multimem insts is 19 ms
      - Coalescing & reducing thread blocks has little benefit (17 ms)
      - But it is nowhere close to theoretical max
    
    This tells us an important lesson:
    - A single H100 cannot issue multimem instructions fast enough to saturate NVLink/SHARP bandwidth
    - Multiple devices must issue multimem instructions in parallel to saturate NVLink/SHARP bandwidth
*/
...
int main() {
    ...
    */
    ...
    printf("Time for %zu MiB plain load-store: %f ms\n", SIZE / 1024 / 1024, elapsed.count() * 1e3);
    ...
    gang.execute([](int dev_id) { // set devices
        CUDACHECK(cudaSetDevice(dev_id));
    });
    gang.execute([](int dev_id) { // check devices (sanity check for myself)
        int dev;
    ...
    // MultiGPU, threadpool version
    for (int iter = 0; iter < NUM_ITERS; ++iter) {
        gang.execute([&nelem, &mcPtrs](int dev_id) { 
            int nelem_per_dev = nelem / NUM_DEVICES;
            int offset = nelem_per_dev * dev_id;
            all_reduce_float32_sum_coalesced<<<(nelem_per_dev + 1024 * STRIDE - 1) / (1024 * STRIDE), 256>>>((float*)(mcPtrs[dev_id]) + offset, nelem_per_dev);
            CUDACHECK(cudaDeviceSynchronize());
        });
    }
    ...
    printf("Time for %zu MiB allreduce using multimem: %lf ms\n", SIZE / (1024 * 1024), avg_time * 1e3);
    ...
    elapsed = end - start;
    printf("Time for %zu MiB load-store after multimem: %f ms\n", SIZE / 1024 / 1024, elapsed.count() * 1e3);
    CUDACHECK(cudaFree(dummy));
