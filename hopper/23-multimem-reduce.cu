...
int main() {
    ...
    */
    ...
    all_reduce_float32_sum<<<(nelem + 255) / 256, 256>>>((float*)mcPtrs[0], nelem);
    CUDACHECK(cudaDeviceSynchronize());
...
__global__ void all_reduce_float32_sum(float *data, int nelem) {
    ...
    if (idx < nelem) {
        volatile float val;
        float *ptr = data + idx;
        asm volatile("multimem.ld_reduce.relaxed.sys.global.add.f32 %0, [%1];" : "=f"(val) : "l"(ptr) : "memory"); // relaxed vs weak?
        asm volatile("fence.proxy.alias;" ::: "memory"); // force memory ordering
        // *ptr = val;
        asm volatile("multimem.st.relaxed.sys.global.f32 [%0], %1;" :: "l"(ptr), "f"(val) : "memory"); // curious: what if I don't use asm here and just use plain assignment?
        /*
