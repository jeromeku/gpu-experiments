...
void run(size_t M, size_t N, size_t K) {
    ...
    // Device-side matrices
    __nv_bfloat16 *device_A, *device_B, *device_C;
    CUDACHECK(cudaMalloc(&device_A, M * K * sizeof(__nv_bfloat16)));
    CUDACHECK(cudaMalloc(&device_B, K * N * sizeof(__nv_bfloat16)));
    CUDACHECK(cudaMalloc(&device_C, M * N * sizeof(__nv_bfloat16)));

    // Convert & copy to device from host
    ...
    CUDACHECK(cudaMemcpy(device_A, host_A_bf16, M * K * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    CUDACHECK(cudaMemcpy(device_B, host_B_bf16, K * N * sizeof(__nv_bfloat16), cudaMemcpyHostToDevice));
    ...
    CUDACHECK(cudaFuncSetAttribute(kittens::prototype::lcf::kernel<mmt>, 
                                   ...
                                   smem_size)); // this sets the maximum **allowed** (<<<..., ..., this>>> sets actual dynamic smem)
    dim3 grid(mmt::grid(M, N, K));
    dim3 block(kittens::prototype::detail::NUM_THREADS_v<mmt>);
    for (int i = 0; i < 2; ++i) // warpup
        inner_run<mmt>(device_A, device_B, device_C, M, N, K, grid, block);
    CUDACHECK(cudaDeviceSynchronize());
    
    // Start timing
    std::cout << "\n  Launching kernel with grid (" << grid.x << ", " << grid.y << "), block (" << block.x << ")\n";
    ...
    CUDACHECK(cudaMemcpy(host_C_bf16, device_C, M * N * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost));
