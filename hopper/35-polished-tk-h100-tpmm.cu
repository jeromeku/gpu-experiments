/*
    ...
    Large matrix, 
------------------------- Benchmark -------------------------
  M = 40960, N = 40960, K = 40960
  Block size: 128x256

  Matrix A (M x K): 0.296543 -0.316565 0.279691 0.0968502 -0.0541672 -0.400025 -0.0407511 -0.166291 -0.357133 0.150888 
  Matrix B (K x N): 0.457442 0.31304 0.00256767 -0.404886 0.445009 0.218401 -0.361566 0.227262 0.206775 0.450321 
  Expected C (M x N): 7.38439 16.6001 -14.6296 -37.0935 -12.55 -6.08156 11.5893 21.4968 -9.66417 -12.0654 

  Launching kernels with grid (132, 1), block (384) on all devices
    Execution time: 35.8089 ms
  Matrix C (M x N): 7.4375 16.625 -14.5625 -37 -12.5 -6.125 11.625 21.5 -9.625 -12.0625 
    Maximum error: 0.220444
    Error count (out of 10x10): 0
-------------------------------------------------------------
...
void run(size_t M, size_t N, size_t K) {
    ...
    */
    ...
    mc_object<__nv_bfloat16> *device_C_mc;
    mc_malloc<__nv_bfloat16>(&device_C_mc, device_ids, NUM_DEVICES, M * N * sizeof(__nv_bfloat16));

    // Prepare kernel launch
    KittenGang gang(device_ids, NUM_DEVICES);
    ...
    for (int i = 0; i < 2; ++i) { // warmup
        gang.execute([&](int dev_idx) { // warmup
            inner_run<mmt>(device_A_sh[dev_idx], device_B_sh[dev_idx], device_C_mc[dev_idx].raw_mem_pointer_, M, N, K_sh, grid, block);
            CUDACHECK(cudaDeviceSynchronize());
        });
        mc_all_reduce_sum_bf16(gang, device_C_mc, NUM_DEVICES);
    }
    ...
    int random_dev_idx = 3; // Should work on any device
    CUDACHECK(cudaSetDevice(random_dev_idx));
    CUDACHECK(cudaMemcpy(host_C_bf16, device_C_mc[random_dev_idx].raw_mem_pointer_, M * N * sizeof(__nv_bfloat16), cudaMemcpyDeviceToHost));
    ...
    for (int dev_idx = 0; dev_idx < NUM_DEVICES; ++dev_idx) {
        CUDACHECK(cudaSetDevice(dev_idx));
        CUDACHECK(cudaFree(device_A_sh[dev_idx]));
        CUDACHECK(cudaFree(device_B_sh[dev_idx]));
    }
    // CUDACHECK(cudaFree(device_C));
    mc_free<__nv_bfloat16>(device_C_mc, NUM_DEVICES);
}
